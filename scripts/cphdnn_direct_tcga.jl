...
for iter_id in 1:50
cph_nb_hl, hlsize,modeltype,nepochs= 2, 512, "direct_cphdnn", 2_000

base_params = Dict(
        ## run infos 
        "session_id" => session_id, "nfolds" =>5,
        "machine_id"=>strip(read(`hostname`, String)), "device" => "$(device())", "model_title"=>"direct_cphdnn_with_tcga_datasets",
        ## optim infos 
...
for DS_name in keys(tcga_datasets) 
    params_dict = tcga_datasets[DS_name]["params"]
    tcga_datasets[DS_name]["model"] = gpu(Chain(Dense(params_dict["insize"],params_dict["cph_hl_size"], leakyrelu), 
    Dense(params_dict["cph_hl_size"],params_dict["cph_hl_size"], leakyrelu), 
    Dense(params_dict["cph_hl_size"], 1, bias = false)))
