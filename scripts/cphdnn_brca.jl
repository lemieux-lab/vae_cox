...
function data_prep(DATA;nfolds = 5, nepochs =2000, dim_redux= 125)
    ...
    params_dict = Dict(
            ...
            ## data infos 
            "dataset" => "BRCA", "nsamples" => size(DATA.samples)[1],
...
for rep_id in 1:50
    train_x, train_y_t, train_y_e, NE_frac_tr, test_x, test_y_t, test_y_e, NE_frac_tst, params_dict = data_prep(BRCA)
    # create model 
    #model = build_vaecox(params_dict)
    enc = gpu(Chain(Dense(size(train_x)[1], 3040, leakyrelu), Dense(3040, 616, leakyrelu), Dense(616,125)))
    ...
    for i in 1:params_dict["nepochs"]
        ...
        gs1 = gradient(ps1) do 
            cox_nll_vec(enc, cphdnn, train_x, train_y_e, NE_frac_tr) + l2_penalty(cphdnn) * wd + l2_penalty(enc) * wd
            #VAE_COX_loss(model["venc"], model["cph"], train_x, train_y_e, NE_frac_tr) + l2_penalty(model["venc"]) * wd + l2_penalty(model["cph"]) * wd 
        end 
        cphloss = round(cox_nll_vec(enc, cphdnn, train_x, train_y_e, NE_frac_tr) + l2_penalty(cphdnn) * wd + l2_penalty(enc) * wd, digits = 3)
        ...
        OUTS_tr = cphdnn(enc(train_x))
        OUTS_tst = cphdnn(enc(test_x))
