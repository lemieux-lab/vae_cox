...
lines!(ax1, SMA(Xs, Ys, k=K,n=N)..., color = "blue", label = "CPHDNN 16 CF", linewidth=3, linestyle =:dash)

## BRCA DNN NO CLINF - GE RDM DIM SWEEP  
Xs, Ys = log10.(cphdnn[cphdnn[:,"nb_clinf"] .== 0 ,"insize"]), cphdnn[cphdnn[:,"nb_clinf"] .== 0,"cph_test_c_ind"]
scatter!(ax1, Xs, Ys, color = "blue")
lines!(ax1, SMA(Xs, Ys, k=K,n=N)..., color = "blue", label = "CPHDNN 0 CF", linewidth=3)
...
lines!(ax1, SMA(Xs, Ys, k=K,n=N)..., color = "orange", label = "Cox-ridge 16 CF", linewidth=3, linestyle=:dash)


## BRCA RIDGE-CPH NO CLINF - GE RDM DIM SWEEP  
Xs, Ys = log10.(coxridge[coxridge[:,"nb_clinf"] .== 0 ,"insize"]), coxridge[coxridge[:,"nb_clinf"] .== 0,"cph_test_c_ind"]
scatter!(ax1, Xs, Ys, color = "orange")
lines!(ax1, SMA(Xs, Ys, k=K,n=N)..., color = "orange", label = "Cox-ridge 0 CF", linewidth=3)
...
lines!(ax3, SMA(Xs, Ys, k=K,n=N)..., color = "blue", label = "CPHDNN 16 CF", linewidth=3, linestyle=:dash)

## BRCA DNN CLINF + GE PCA DIM SWEEP  
Xs, Ys = log10.(cphdnn[cphdnn[:,"nb_clinf"] .== 0 ,"insize"]), cphdnn[cphdnn[:,"nb_clinf"] .== 0,"cph_test_c_ind"]
scatter!(ax3, Xs, Ys, color= "blue")
lines!(ax3, SMA(Xs, Ys, k=K,n=N)..., color = "blue", label = "CPHDNN 0 CF", linewidth=3)

## BRCA Cox-Rdige NO CLINF - GE PCA DIM SWEEP  
Xs, Ys = log10.(coxridge[coxridge[:,"nb_clinf"] .!= 0 ,"insize"]), coxridge[coxridge[:,"nb_clinf"] .!= 0,"cph_test_c_ind"]
scatter!(ax3, Xs, Ys, color = "orange", alpha=0.5)
lines!(ax3, SMA(Xs, Ys, k=K,n=N)..., color = "orange", label = "Cox-ridge 16 CF", linewidth=3, linestyle=:dash)

## BRCA DNN NO CLINF - GE PCA DIM SWEEP  
Xs, Ys = log10.(coxridge[coxridge[:,"nb_clinf"] .== 0 ,"insize"]), coxridge[coxridge[:,"nb_clinf"] .== 0,"cph_test_c_ind"]
scatter!(ax3, Xs, Ys, color= "orange")
lines!(ax3, SMA(Xs, Ys, k=K,n=N)..., color = "orange", label = "Cox-ridge 0 CF",linewidth=3)
...
lines!(ax2, SMA(Xs, Ys, k=K,n=N)..., color = "blue", label = "CPHDNN 8 CF", linewidth=3, linestyle=:dash)

## LGN CPHDNN NO CLINF - GE DIM SWEEP  
Xs, Ys = log10.(cphdnn[cphdnn[:,"nb_clinf"] .== 0 ,"insize"]), cphdnn[cphdnn[:,"nb_clinf"] .== 0,"cph_test_c_ind"]
scatter!(ax2, Xs, Ys, color = "blue")
lines!(ax2, SMA(Xs, Ys, k=K,n=N)..., color = "blue", label = "CPHDNN 0 CF", linewidth=3)

## LGN Cox-ridge CLINF + GE DIM SWEEP  
Xs, Ys = log10.(coxridge[coxridge[:,"nb_clinf"] .!= 0 ,"insize"]), coxridge[coxridge[:,"nb_clinf"] .!= 0,"cph_test_c_ind"]
scatter!(ax2, Xs, Ys, color= "orange", alpha = 0.5)
lines!(ax2, SMA(Xs, Ys, k=K,n=N)..., color = "orange", label = "Cox-ridge 8 CF", linewidth=3, linestyle=:dash)

## LGN Cox-ridge NO CLINF + GE DIM SWEEP  
Xs, Ys = log10.(coxridge[coxridge[:,"nb_clinf"] .== 0 ,"insize"]), coxridge[coxridge[:,"nb_clinf"] .== 0,"cph_test_c_ind"]
scatter!(ax2, Xs, Ys, color= "orange")
lines!(ax2, SMA(Xs, Ys, k=K,n=N)..., color = "orange", label = "Cox-ridge 0 CF", linewidth=3)
...
lines!(ax4, SMA(Xs, Ys, k=K,n=N)..., color = "blue", label = "CPHDNN 16 CF", linewidth=3, linestyle=:dash)
fig
## BRCA DNN CLINF + GE PCA DIM SWEEP  
Xs, Ys = log10.(cphdnn[cphdnn[:,"nb_clinf"] .== 0 ,"insize"]), cphdnn[cphdnn[:,"nb_clinf"] .== 0,"cph_test_c_ind"]
scatter!(ax4, Xs, Ys, color= "blue")
lines!(ax4, SMA(Xs, Ys, k=K,n=N)..., color = "blue", label = "CPHDNN 0 CF", linewidth=3)

## BRCA Cox-Rdige NO CLINF - GE PCA DIM SWEEP  
Xs, Ys = log10.(coxridge[coxridge[:,"nb_clinf"] .!= 0 ,"insize"]), coxridge[coxridge[:,"nb_clinf"] .!= 0,"cph_test_c_ind"]
scatter!(ax4, Xs, Ys, color = "orange", alpha=0.5)
lines!(ax4, SMA(Xs, Ys, k=K,n=N)..., color = "orange",label = "Cox-ridge 16 CF", linewidth=3, linestyle=:dash)

## BRCA DNN NO CLINF - GE PCA DIM SWEEP  
Xs, Ys = log10.(coxridge[coxridge[:,"nb_clinf"] .== 0 ,"insize"]), coxridge[coxridge[:,"nb_clinf"] .== 0,"cph_test_c_ind"]
scatter!(ax4, Xs, Ys, color= "orange")
lines!(ax4, SMA(Xs, Ys, k=K,n=N)..., color = "orange", label = "Cox-ridge 0 CF", linewidth=3)
