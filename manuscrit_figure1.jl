...
ngenes = sum(keep)
println("$dataset_name nb genes : $(ngenes)")
...
hlsize = 512
cph_nb_hl, modeltype, nepochs= 2,  "cphdnn", 10000
# for dim size in list do 
dim_redux_size = 15#dim_redux_list[end]
nb_clinf = 0
...
base_params = Dict(
        ...
        ## optim infos 
        "nepochs" => nepochs, "ae_lr" =>0, "cph_lr" => 1e-3, "ae_wd" => 0, "cph_wd" => 0.00001,
        ## model infos
...
# split folds 
folds = split_train_test(dredux_data, DATA.survt, DATA.surve, DATA.samples;nfolds =DataDict["params"]["nfolds"])
OUTS_TST = []
Y_T_TST = []
Y_E_TST = []
